...
class ProgramGraphSlicer:
        ...
        for path, tree in self.ast_trees.items():
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    ...
                    # If it's a class method, qn is "class_name.method_name"; otherwise just "method_name"
                    if cls:
    ...
    def _find_defs_uses(self, path: str, var: str) -> Tuple[Set[ast.AST], Set[ast.AST]]:
        ...
        # Function parameters are also defs
    ...
    def _build_edges(self, path: str, var: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        # Backward (use -> def), Forward (def -> use)
        for u in uses:
            for d in defs:
    ...
    def _gather_nodes(self, path: str, slice_nodes: Set[ast.AST]) -> List[Dict[str, Any]]:

        nodes: Dict[str, Dict[str, Any]] = {}
        pm = self.parent_maps[path]
        lines = self.source_by_file[path].splitlines()
        for n in slice_nodes:
            key = self._node_key(n)
            if key in nodes: continue
            typ = 'statement'
            if isinstance(n, ast.FunctionDef): typ = 'function'
            if isinstance(n, ast.Call): typ = 'call'
            if isinstance(n, ast.Name): typ = 'variable'
            if isinstance(n, ast.Return): typ = 'return'
            if isinstance(n, ast.If): typ = 'if'
            if isinstance(n, ast.While): typ = 'while'
            if isinstance(n, ast.For): typ = 'for'
            node_file = path
            for file_path in self.ast_trees:
                if any(n is node for node in ast.walk(self.ast_trees[file_path])):
                    node_file = file_path
                    break
            file_lines = self.source_by_file[node_file].splitlines()
            code = ''
            if hasattr(n, 'lineno') and n.lineno > 0:
                try:
                    if n.lineno <= len(file_lines):
                        code = file_lines[n.lineno-1].strip()
                except IndexError:
                    code = f"<error: line {n.lineno} exceeds file length>"
            fn = None
            cls = None
            if node_file == path:
                fn = self._find_enclosing(n, pm, ast.FunctionDef)
                cls = self._find_enclosing(n, pm, ast.ClassDef)
            else:
                node_pm = self.parent_maps[node_file]
                fn = self._find_enclosing(n, node_pm, ast.FunctionDef)
                cls = self._find_enclosing(n, node_pm, ast.ClassDef)
            nodes[key] = {
                'id': key,
                'type': typ,
                'code': code,
                'variable': n.id if isinstance(n, ast.Name) else None,
                'source_function': fn.name if fn else None,
                'source_class': cls.name if cls else None,
                'source_file': node_file,
                'line': n.lineno if hasattr(n, 'lineno') else None,
                'col_offset': getattr(n, 'col_offset', None)   # 新增列偏移
            }
    ...
    def perform_slicing(self):
        results = []
        visited_funcs = set()    # Recursive deduplication, prevent processing the same function multiple times
        eid_counter = [0]        # Use mutable list to store global auto-incrementing EID
        for qn in self.func_index:
            self._collect_func_and_calls(qn, results, visited_funcs, eid_counter)
        return results

    def _collect_func_and_calls(self, qn, results, visited_funcs, eid_counter):
        if qn in visited_funcs:
            return
        visited_funcs.add(qn)
        path, func_node = self.func_index[qn]
        lines = self.source_by_file[path].splitlines()
        code = "\n".join(lines[func_node.lineno-1:func_node.end_lineno])
        pm = self.parent_maps[path]
        cls = self._find_enclosing(func_node, pm, ast.ClassDef)
        ...
        for node in ast.walk(func_node):
            if isinstance(node, ast.Name) and isinstance(node.ctx, ast.Store):
                var_names.add((node.id, node.lineno))
        slice_list = []
        for var, ln in var_names:
            ...
            b_nodes = self._expand_slice_with_cfg_cg_recursive(path, b_nodes, True)
            f_nodes = self._expand_slice_with_cfg_cg_recursive(path, f_nodes, False)
            all_nodes = {n for n in ast.walk(func_node) if isinstance(n, ast.stmt)}

            def id2node(node_id):
                for file_path, tree in self.ast_trees.items():
                    for node in ast.walk(tree):
                        if self._node_key(node) == node_id:
                            return node
                return None

            edge_nodes = set()
            for edge in fwd_edges + bwd_edges:
                src_node = id2node(edge['src'])
                dst_node = id2node(edge['dst'])
                if src_node: edge_nodes.add(src_node)
                if dst_node: edge_nodes.add(dst_node)
            all_nodes.update(edge_nodes)

            node_label = {}
            for n in f_nodes:
                node_label[self._node_key(n)] = 'forward'
            for n in b_nodes:
                node_label[self._node_key(n)] = 'backward'
            for n in all_nodes:
                if self._node_key(n) not in node_label:
                    node_label[self._node_key(n)] = 'none'

            nodes_struct = []
            for n in all_nodes:
                info = self._gather_nodes(path, {n})[0]
                info['slice_label'] = node_label[self._node_key(n)]
                nodes_struct.append(info)

            slice_list.append({
                'slice_criterion': {'variable': var, 'line': ln},
                'nodes': nodes_struct,
                'edges': fwd_edges + bwd_edges,
            })
        eid = eid_counter[0]
        eid_counter[0] += 1
        results.append({
        ...
        for edge in self.cg_edges[path]:
            if edge['src'] == self._node_key(func_node):
                for qn2, (callee_path, callee_node) in self.func_index.items():
                    if self._node_key(callee_node) == edge['dst']:
                        self._collect_func_and_calls(qn2, results, visited_funcs, eid_counter)


    def export_json(self, output: str):
        """
        Export slice data to a JSON file for downstream applications like GNN training/analysis
        """
