...
def process_project(project, input_dir, output_dir, slicer_path, additional_args, timeout):
    ...
    output_json = os.path.join(project_output_path, "llm_slices.json")
    cmd = [sys.executable, slicer_path, '-d', project_input_path, '-o', output_json, '--mode', 'var']
    if additional_args:
...
def merge_slices(slices_dir, output_dir, output_name="llm_slices"):
    ...
    for split in splits:
        ...
        for project in tqdm(projects, desc=f"Merging {split} data"):
            slices_json_path = os.path.join(split_dir, project, "llm_slices.json")
        ...
        output_file = os.path.join(output_dir, f"{output_name}_{split}.json")
        try:
...
def main():
    parser = argparse.ArgumentParser(description='Batch run llm_verification.py for all Python projects in dataset directories')
    ...
    parser.add_argument('-o', '--output', default='./llm_slices', help='Output directory for slicing results (default: llm_slices)')
    parser.add_argument('-s', '--slicer', default='./llm_test/verification/llm_veryfication.py', help='Path to llm_verification.py script (default: ./llm_verification.py)')
    parser.add_argument('--additional-args', nargs=argparse.REMAINDER, help='Additional arguments to pass to llm_verification.py')
    parser.add_argument('-v', '--verbose', action='store_true', help='Enable verbose logging')
    parser.add_argument('-t', '--timeout', type=int, default=3600, help='Timeout in seconds for processing each project (default: 36000 seconds)')
    ...
    if args.merge:
        ...
        merge_slices(args.output, args.merge_dir, output_name="llm_slices")
